...
for i=1:numiter
    ...
    for q_i= 1:q_len
        ...
        for n_i = 1:n_len
            ...
            tic;
            matching=ConVogHard_hybrid_order(A,B,n_val_for_i,ordering);
           ...
           matching_rqap=ConVogHard_rQAP_order(A,B,n_val_for_i,ordering,1);
            
            %             P_l2_cell{n_i,i}=P_l2(:,ordering);
            %             P_l2_pr_cell{n_i,i}=P_proj_l2(:,ordering);
            %
           corr_match_rqap(n_i,i,q_i) =  sum(matching_rqap((n_val_for_i+1):N)==ordering((n_val_for_i+1):N));
            ...
            matching_unseed=ConVogHard_hybrid(A_sub,B_sub,0);
...
pc_rqap = corr_match_rqap;
fc_rqap= pc_rqap./repmat((N-n_vals'),[1 numiter length(q)]);



pc_unseed=corr_match_unseed;
fc_unseed= pc_unseed./repmat((N-n_vals'),[1 numiter length(q)]);
fname = strcat('./cache/hybrid/sim_bitflip_', datestr(clock) ,int2str(uint32(randi(1E6,1))), '.mat')
save(fname)

...
corr_match_exact = zeros(n_len,numiter,q_len);
...
for i=1:numiter
    ...
    for q_i= 1:q_len
        ...
        for n_i = 1:n_len
            ...
            tic;
            [alignment_rqap, iter, ~,fval_JV,fval_proj_JV,P_jv,P_proj_jv]=ConVogHard_rQAP_order(A,B,n_val_for_i,ordering,1);
             [alignment_from_exact_soln]= GM_exact(A(ordering,ordering),B(ordering,ordering),n_val_for_i);
             exact_matched_v= ordering(alignment_from_exact_soln(test_ind));
             
            %P_jv_cell{n_i,i}=P_jv(:,ordering);
            
            %P_jv_pr_cell{n_i,i}=P_proj_jv(:,ordering);
            %running_time_FAQ(i,n_i,q_i)=toc;
            matched_v=alignment_rqap(test_ind);
            corr_match(n_i,i,q_i) =  sum(matched_v==test_v);
            corr_match_exact(n_i,i,q_i) = sum(exact_matched_v==test_v);
...
fc= pc./repmat((N-n_vals'),[1 numiter length(q)]);
 ...
 pc_exact = corr_match_exact;
 
fc_exact= pc_exact./repmat((N-n_vals'),[1 numiter length(q)]);
